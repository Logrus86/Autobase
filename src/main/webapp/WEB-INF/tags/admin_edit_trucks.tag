...
<fmt:bundle basename="i18n.text">
    <div id="trucks">
        <table>
                        ...
                        <td>
                            <select class="selectpicker show-menu-arrow" data-width="150px" data-live-search="true" name="model">
                                <c:forEach items="${modelList}" var="model">
                                    <option
                                            <c:if test="${vehicle.model==model.value}">selected</c:if>>${model.value}</option>
                                </c:forEach>
                            </select>
                        </td>
                        <td>
                            <select class="selectpicker show-menu-arrow" data-width="150px" data-live-search="true" name="manufacturer">
                                <c:forEach items="${manufacturerList}" var="manufacturer">
                                    <option
                                            <c:if test="${vehicle.manufacturer==manufacturer.value}">selected</c:if>>${manufacturer.value}</option>
                                </c:forEach>
                            </select>
                        </td>
                        <td><input type="number" class="form-control" name="productionYear" value="${vehicle.productionYear}"></td>
                        <td>
                            <select class="selectpicker show-menu-arrow" data-width="150px" data-live-search="true" name="color">
                                <c:forEach items="${colorList}" var="color">
                                    <option
                                            <c:if test="${vehicle.color==color.valueEn}">selected</c:if>
                                            value="${color.valueEn}">${color.valueRu}</option>
                                </c:forEach>
                            </select>
                        </td>
                        <td>
                            <select class="selectpicker show-menu-arrow" data-width="auto" name="fuelType">
                                <option value="PETROL" <c:if test="${vehicle.fuelType=='PETROL'}">selected</c:if>>
                                    <fmt:message key="default.petrol"/></option>
                                <option value="DIESEL" <c:if test="${vehicle.fuelType=='DIESEL'}">selected</c:if>>
                                    <fmt:message key="default.diesel"/></option>
                                <option value="GAS" <c:if test="${vehicle.fuelType=='GAS'}">selected</c:if>><fmt:message
                                        key="default.gas"/></option>
                                <option value="GAS_PETROL"
                                        <c:if test="${vehicle.fuelType=='GAS_PETROL'}">selected</c:if>><fmt:message
                                        key="default.gas-petrol"/></option>
                                <option value="ELECTRICITY"
                                        <c:if test="${vehicle.fuelType=='ELECTRICITY'}">selected</c:if>><fmt:message
                                        key="default.electricity"/></option>
                            </select>
                        </td>
            ...
            <tr>
                <td><button class="btn btn-success" name="add-truck" type="button"><fmt:message key="default.add"/></button></td>
            </tr>
        </table>
    </div>
</fmt:bundle>