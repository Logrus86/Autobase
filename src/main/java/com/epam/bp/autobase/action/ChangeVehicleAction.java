...
public class ChangeVehicleAction implements Action {
    ...
    private ActionResult admin_vehicles = new ActionResult("admin-vehicles", true);
            ...
            try {
                ...
                daoManager.transactionAndClose(new H2DaoManager.DaoCommand() {
                    ...
                    public void execute(H2DaoManager daoManager) throws DaoException {
                        ...
                        Integer passSeatsNum = Integer.parseInt(request.getParameter(PASS_SEATS_NUM));
                        Integer doorsNum = Integer.parseInt(request.getParameter(DOORS_NUM));
                        Integer standPlacesNum = Integer.parseInt(request.getParameter(STAND_PLACES_NUM));
                        Boolean withConder = ON.equals(request.getParameter(WITH_COND));
                        BigDecimal maxPayload = new BigDecimal(request.getParameter(MAX_PAYLOAD));
                        Boolean enclosed = ON.equals(request.getParameter(ENCLOSED));
                        Boolean tipper = ON.equals(request.getParameter(TIPPER));
